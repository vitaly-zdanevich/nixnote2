...
// Permanently delete resource files of a list of notes
void ResourceTable::expunge(const QList<qint32> &lids) {
    QString slids = "";
    for (int i = 0; i < lids.size(); ++i) {
        if (this->exists(lids[i])) {
            slids += lids[i] + ",";
        }
    }
    if (slids == "") {
    ...
    for (int i = 0; i < lids.size(); ++i) {
        ...
        QString num = QString::number(lids[i]);
        QStringList filter;
